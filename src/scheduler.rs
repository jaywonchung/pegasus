...
///
/// Slots are numbered 0 to `total_slots - 1`. The allocator prefers
/// contiguous blocks starting at even indices to respect NVLink topology
/// (GPUs 0-1 and 2-3 are typically NVLink-connected).
#[derive(Debug)]
...
impl HostSlotState {
    ...
    /// Attempts to allocate `n` slots using NVLink-aware allocation.
    ///
    /// Priority order:
...
pub fn find_host_for_job(
    ...
    for (i, host) in hosts.iter_mut().enumerate() {
        if let Some(slots) = host.allocate(slots_required) {
            return Some((i, slots));
        }
    }
    None
...
mod tests {
    ...
    fn test_allocate_single_slot() {
        ...
        let slots = state.allocate(1).unwrap();
    ...
    fn test_allocate_all_slots() {
        let mut state = HostSlotState::new(8);
        let slots = state.allocate(8).unwrap();
    ...
    fn test_allocate_insufficient_slots() {
        ...
        assert!(state.allocate(5).is_none());
    ...
    fn test_allocate_prefers_even_start_for_2_slots() {
        let mut state = HostSlotState::new(8);
        assert_eq!(state.allocate(2).unwrap(), vec![0, 1]);
        assert_eq!(state.allocate(2).unwrap(), vec![2, 3]);
        assert_eq!(state.allocate(2).unwrap(), vec![4, 5]);
        assert_eq!(state.allocate(2).unwrap(), vec![6, 7]);
    }

    #[test]
    fn test_allocate_4_slots_even_aligned() {
        let mut state = HostSlotState::new(8);
        assert_eq!(state.allocate(4).unwrap(), vec![0, 1, 2, 3]);
        assert_eq!(state.allocate(4).unwrap(), vec![4, 5, 6, 7]);
    }

    #[test]
    fn test_allocate_falls_back_to_even_start_after_release() {
        let mut state = HostSlotState::new(8);
        state.allocate(2); // 0,1
        state.allocate(2); // 2,3
        state.allocate(2); // 4,5
        state.release(&[2, 3]);
    ...
    fn test_allocate_non_contiguous_fallback() {
        ...
        let slots = state.allocate(3).unwrap();
    ...
    fn test_release_slots() {
        let mut state = HostSlotState::new(8);
        let slots = state.allocate(4).unwrap();
        assert_eq!(state.free_slots(), 4);
        state.release(&slots);
    ...
    fn test_interleaved_allocate_release() {
        let mut state = HostSlotState::new(8);
        let a = state.allocate(2).unwrap(); // 0,1
        let b = state.allocate(2).unwrap(); // 2,3
        let c = state.allocate(2).unwrap(); // 4,5
    ...
    fn test_allocate_zero_slots() {
        let mut state = HostSlotState::new(8);
        let slots = state.allocate(0).unwrap();
    ...
    fn test_single_slot_host() {
        ...
        let a = state.allocate(1).unwrap();
        assert_eq!(a, vec![0]);
        assert!(state.allocate(1).is_none());
        state.release(&a);
        assert_eq!(state.allocate(1).unwrap(), vec![0]);
    }
    ...
    fn test_find_host_for_job_simple() {
        ...
        let (host_idx, slots) = find_host_for_job(&mut hosts, 4).unwrap();
    ...
    fn test_find_host_for_job_first_full() {
        let mut hosts = make_hosts(&[8, 8]);
        hosts[0].allocate(8); // fill first host
        let (host_idx, slots) = find_host_for_job(&mut hosts, 4).unwrap();
    ...
    fn test_find_host_for_job_heterogeneous() {
        ...
        let (host_idx, slots) = find_host_for_job(&mut hosts, 6).unwrap();
        assert_eq!(host_idx, 1);
    ...
    fn test_find_host_for_job_none_available() {
        ...
        assert!(find_host_for_job(&mut hosts, 3).is_none());
    }
    ...
    fn test_concurrent_jobs_same_host() {
        ...
        let (_, slots1) = find_host_for_job(&mut hosts, 2).unwrap();
        let (_, slots2) = find_host_for_job(&mut hosts, 2).unwrap();
        let (_, slots3) = find_host_for_job(&mut hosts, 2).unwrap();
    ...
    fn test_release_enables_new_allocation() {
        ...
        let (_, slots1) = find_host_for_job(&mut hosts, 4).unwrap();
        assert!(find_host_for_job(&mut hosts, 1).is_none());
        hosts[0].release(&slots1);
        assert!(find_host_for_job(&mut hosts, 4).is_some());
    }
    ...
    fn test_backwards_compat_default_slots_one_job_per_host() {
        ...
        let (h0, s0) = find_host_for_job(&mut hosts, 1).unwrap();
        assert_eq!(h0, 0);
        assert_eq!(s0, vec![0]);

        let (h1, s1) = find_host_for_job(&mut hosts, 1).unwrap();
        assert_eq!(h1, 1);
        assert_eq!(s1, vec![0]);

        let (h2, s2) = find_host_for_job(&mut hosts, 1).unwrap();
        assert_eq!(h2, 2);
        assert_eq!(s2, vec![0]);
        ...
        let (h3, _) = find_host_for_job(&mut hosts, 1).unwrap();
        assert_eq!(h3, 1);
    }
    ...
    fn test_backwards_compat_no_concurrent_jobs_on_single_slot_host() {
        ...
        let (_, slots) = find_host_for_job(&mut hosts, 1).unwrap();
        ...
        assert!(find_host_for_job(&mut hosts, 1).is_some());
    }
    ...
    fn test_backwards_compat_jobs_default_to_one_slot() {
        ...
        let mut hosts = make_hosts(&[1]);
        let result = find_host_for_job(&mut hosts, 1);
    ...
    fn test_backwards_compat_fifo_order_preserved() {
        ...
        // Job 1 -> Host 0
        let (h, _) = find_host_for_job(&mut hosts, 1).unwrap();
        assert_eq!(h, 0);
